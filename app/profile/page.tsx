...
export default function ProfilePage() {
  ...
  useEffect(() => {
    const loadUserData = () => {
      // 🌟 อัปเกรด: เปลี่ยนมาดึงสถานะจาก sessionStorage
      const status = sessionStorage.getItem("isLoggedIn");
      setIsUserLoggedIn(status === "true");

      const savedImage = localStorage.getItem("profileImage"); // รูปโปรไฟล์ให้เป็น local ไว้ได้เผื่อโหลดไว
      if (savedImage) setProfileImage(savedImage);

      // 🌟 อัปเกรด: เปลี่ยนมาดึงชื่อจาก sessionStorage
      ...
      // ข้อมูลสุขภาพยังดึงจาก localStorage ไว้ก่อนได้ (ถ้าตอนคำนวณเซฟไว้ที่นั่น)
      const savedBMI = localStorage.getItem("userBMI");
      if (savedBMI) setUserBMI(savedBMI);
  ...
  return (
    <div className="min-h-screen bg-[#fafaf9] flex flex-col font-sans pb-24 relative overflow-hidden">
      ...
      {/* 🌟 ของตกแต่งพื้นหลังให้ดูล้ำๆ มีมิติ */}
      ...
      <main className="flex-grow w-full max-w-4xl mx-auto pt-12 px-6">
        ...
        <header className="mb-10 flex justify-between items-end">
          <div>
            ...
            <p className="text-gray-500 font-medium mt-2">จัดการข้อมูลส่วนตัว</p>
          </div>
        </header>
        ...
        <div className="grid grid-cols-1 md:grid-cols-12 gap-6 auto-rows-[minmax(180px,auto)]">
          ...
          {/* บล็อก 1: Profile (ใหญ่สุด ซ้ายบน) */}
          ...
          <Link href="/my-fridge" className="md:col-span-4 bg-gradient-to-br from-[#f26522] to-orange-500 p-8 rounded-[2rem] shadow-md border border-orange-400 flex flex-col justify-between group relative overflow-hidden transition-all hover:shadow-lg hover:-translate-y-1">
            ...
            <div className="mt-6 flex items-center justify-between text-white/90">
              <span className="text-sm font-bold bg-white/20 px-3 py-1.5 rounded-full backdrop-blur-sm">AI จัดการวัตถุดิบ</span>
          ...
          {/* บล็อก 5: สุขภาพ AI (ขวาล่าง หรือ แถวล่างสุดถ้าย่อจอ) */}
          <div className="md:col-span-12 lg:col-span-4 bg-gray-900 p-8 rounded-[2rem] shadow-md border border-gray-800 flex flex-col justify-between text-white relative overflow-hidden">
             ...
             <div className="flex items-center justify-between mb-6">
                <div className="flex items-center gap-3">
                  ...
                  <h3 className="font-bold text-gray-200">สถานะสุขภาพ AI</h3>
                </div>
             </div>
             ...
             {userBMI ? (
               <div className="space-y-4">
                 <div className="flex justify-between items-end border-b border-gray-700 pb-3">
                   <span className="text-gray-400 text-sm font-medium">BMI</span>
                   <div className="text-right">
                     <span className="text-2xl font-black text-white">{userBMI}</span>
                     <span className="block text-[#f26522] text-xs font-bold mt-1">{userBMIStatus}</span>
                   </div>
                 </div>
                 <div className="flex justify-between items-end">
                   <span className="text-gray-400 text-sm font-medium">TDEE</span>
                   <div className="text-right">
                     <span className="text-2xl font-black text-white">{userTDEE}</span>
                     <span className="text-gray-500 text-xs font-medium ml-1">kcal</span>
                   </div>
                 </div>
               </div>
             ) : (
               <div className="text-center py-4">
                 <p className="text-gray-400 text-sm mb-4">ยังไม่มีข้อมูลให้ AI ประมวลผล</p>
                 <button onClick={() => router.push('/calculate')} className="w-full bg-[#f26522] hover:bg-orange-600 text-white font-bold py-3 px-4 rounded-xl transition-all active:scale-95 text-sm shadow-sm">
                   เริ่มคำนวณเลย
                 </button>
               </div>
             )}
