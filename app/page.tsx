...
export default function Home() {
  ...
  // 🌟 State สำหรับเช็คสถานะ Login และโชว์ Pop-up
  const [isUserLoggedIn, setIsUserLoggedIn] = useState(false);
  const [showLoginAlert, setShowLoginAlert] = useState(false);
  const [isRandomizing, setIsRandomizing] = useState(false);

 // 🌟 ดึงสถานะตอนโหลดหน้าเว็บ
  useEffect(() => {
    const loadUserData = () => {
      // หน้าแรกมีหน้าที่แค่ "ดึง (getItem)" มาเช็คเฉยๆ ครับ
      const status = sessionStorage.getItem("isLoggedIn");
      setIsUserLoggedIn(status === "true");
    };
    loadUserData();
    
    // ฟังเสียงเผื่อมีการล็อกอินจากส่วนอื่น
    window.addEventListener("profileUpdated", loadUserData);
    return () => window.removeEventListener("profileUpdated", loadUserData);
  }, []);

  // 📡 ฟังก์ชันแอบส่งสถิติไปเก็บบน Database
  ...
  // 🌟 กดปุ่มค้นหาจากวัตถุดิบ (เช็คสิทธิ์ก่อนไป)
  ...
  // 🎲 ระบบสุ่มเมนูอาหาร (เช็คสิทธิ์ก่อนสุ่ม)
          ...
          <div className="bg-white w-80 rounded-[2rem] shadow-2xl py-8 px-6 relative text-center flex flex-col items-center transform transition-transform scale-100">
            ...
            <div className="flex gap-3 w-full">
              ...
              <button 
                ...
                className="flex-1 py-3 text-sm font-bold text-white bg-[#f26522] hover:bg-orange-600 rounded-xl shadow-md transition-colors"
              >
                เข้าสู่ระบบ
              </button>
            </div>
      ...
      <main className="flex flex-col items-center justify-center mt-10 px-4 text-center pb-24 z-10 relative flex-grow">
        ...
        <div className="flex flex-wrap justify-center gap-4 w-full max-w-md">
          <button
            ...
            className="flex-1 bg-[#f26522] hover:bg-orange-600 text-white font-extrabold py-3 px-6 rounded-full shadow-md flex flex-col items-center justify-center text-center leading-tight min-w-[160px] transition-transform hover:scale-105"
          >
          ...
          <button
            ...
            className="flex-1 bg-[#e5e7eb] hover:bg-gray-300 text-gray-800 font-extrabold py-3 px-6 rounded-full shadow-sm min-w-[100px] text-sm md:text-base transition-transform hover:scale-105"
          >
